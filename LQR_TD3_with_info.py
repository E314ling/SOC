...
class experience_memory():
    ...
    def __init__(self, buffer_capacity, batch_size, state_dim, action_dim):
        ...
        self.Q_buffer = np.zeros((self.buffer_capacity, 1), dtype=np.float32)
    ...
    def record(self, obs_tuple):
        ...
        self.Q_buffer[index] = obs_tuple[5]
...
class ActorCritic():
    ...
    @tf.function
    def update_actor(self, state_batch, action_batch, reward_batch, next_state_batch, done_batch, Q_batch):
        
        with tf.GradientTape() as tape:
            tape.watch(self.actor.trainable_variables)
            actions = self.actor(state_batch)
            #critic_value_1,critic_value_2 = self.critic_1([state_batch, actions]), self.critic_2([state_batch, actions])
            actor_loss = tf.math.reduce_mean(Q_batch)
    ...
    def learn(self,frame_num):
        ...
        Q_batch = tf.convert_to_tensor(self.buffer.Q_buffer[batch_indices])
        ...
        if (frame_num % self.update_frames == 0 and frame_num != 0):
            self.update_actor(state_batch, action_batch, reward_batch, next_state_batch, done_batch,Q_batch)
...
class CaseOne():
    ...
    def value_fct(self,n,x):
        P = self.P_t[n]
        Q = self.Q_t[n]
        y = np.dot(np.transpose(x),P)
    ...
    def run_episodes(self, n_x,V_t,A_t, base):
        ...
        for ep in range(self.num_episodes):
            ...
            while(True):
                ...
                Q_val = self.value_fct(n,X[n])
                self.AC.buffer.record((state.numpy()[0],action.numpy(),reward, new_state.numpy()[0], done, Q_val))
    ...
    def dashboard(self,n_x,V_t,A_t,avg_reward_list,AC: ActorCritic,base):
        ...
        ax.hlines(base,xmin = 0, xmax = self.num_episodes, color = 'black', label = 'baseline: {}'.format(np.round(base,)))
        ax.legend()
        ...
        ax.set_title('MSE policy function y = 0 n = {} '.format(t0))
        ax.set_xlim([0,self.num_episodes])
        ax.set_xlabel('Episode')
        ax.legend()
        ...
        ax.set_xlabel('x')
        ax.set_ylabel('y')
        ax.set_title('terminal value function n = {}'.format(self.N))
        
        fig.set_size_inches(w = 18, h= 8)
        fig.tight_layout()
        plt.subplots_adjust(wspace=0.15, hspace=0.3)
        fig.savefig('.\Bilder_SOC\info_LQR_TD3_Episode_{}'.format(len(avg_reward_list)))
        #plt.show()
