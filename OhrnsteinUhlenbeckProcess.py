...
class ActorCritic():
    ...
    def learn(self,frame_num):
        ...
        if (frame_num % 2 == 0 and frame_num != 0):
            for _ in range(2):
    ...
    def get_actor_NN(self):
        ...
        outputs = layers.Dense(self.action_dim, activation='linear', kernel_initializer=last_init)(out)
    ...
    def target_policy(self, state):
        ...
        #noice = self.noice_Obj()
        noice = tf.random.normal(shape = sampled_actions.get_shape(), mean = 0.0, stddev = self.target_var, dtype = tf.float32)
        ...
        for ep in range(num_episodes):
            ...
            while(True):
                ...
                if (n <= self.N-2):
                    ...
                    self.AC.buffer.record((state.numpy()[0],action,reward, new_state.numpy()[0], done_training, terminal_cond))
