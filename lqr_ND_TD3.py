...
class CaseOne():
    ...
    def __init__(self,run):
        ...
        self.f_A = np.identity(self.dim)
        self.f_B = np.identity(self.dim)

        # g(x) = D * ||x||^2
        self.D = np.identity(self.dim)
    ...
    def fill_buffer(self, num_episodes):
        ...
        for ep in range(num_episodes):
            X = np.zeros((self.N,self.dim), dtype= np.float32)
            ...
            while(True):
                ...
                state = np.append(np.array([n], np.float32), X[n])
               
               
                state = tf.expand_dims(tf.convert_to_tensor(state),0)
                done = self.check_if_done(n,state)

                action = tf.convert_to_tensor(2* np.random.rand(self.action_dim) -1)
                ...
                if (done):
                    ...
                    X[0] = self.start_state()
                    new_state = np.append(np.array([0], np.float32), X[0])
                    ...
                    if (self.discrete_problem):
                        ...
                        X[n+1] =  (X[n] + action_env) + self.sig*np.random.normal(size =self.dim)
                    else:
                        X[n+1] =  X[n] + (X[n] + action_env)*self.dt + self.sig*np.sqrt(self.dt)  * np.random.normal(size=self.dim)
                    new_state = np.append(np.array([n+1], np.float32), X[n+1])
                    new_state = tf.expand_dims(tf.convert_to_tensor(new_state),0)
    ...
    def save_all(self, n_x, V_t,A_t,ep_reward_list, avg_reward_list, AC:ActorCritic,base):
        ...
        np.save('./Saved_Runs/LQR/ND_ep_reward_list_{}_run_{}'.format(steps, self.run), np.array(ep_reward_list))
        np.save('./Saved_Runs/LQR/ND_avg_reward_step_{}_run_{}'.format(steps, self.run), np.array(avg_reward_list))
        np.save('./Saved_Runs/LQR/ND_base_run_{}'.format(self.run), base)
...
if __name__ == "__main__":
    ...
    for i in range(0,runs):
        lqr = CaseOne(i)
        V_t,A_t,base = LQR.Solution_N_D(lqr).create_solution(n_x)
